...
Пример:
```
    http.addFilterAfter(new MyCustomFilter(), UsernamePasswordAuthenticationFilter.class);
```
...
```
    ...
    или же можно вынести всю эту реализацию в отдельные классы реализующие интерфейсы authenticationException и accessDeniedException
```
